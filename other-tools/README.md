...
> **Status: 未検証** — 設定例を用意していますが、実際の動作確認はまだ行っていません。設定に誤りがある可能性があります。

以下のツールはプラグイン/フック機能を持ちませんが、MCPサーバーとして接続できます。
speak/statusツールが利用可能になります。

## セットアップ

1. 各ツールのMCP設定に接続先を追加（設定例は各ファイルを参照）
2. デフォルトでクラウドサーバー（`https://spreka.se-es.net`）に接続されます
...
各設定ファイル内のURL（デフォルト: `https://spreka.se-es.net/mcp`）を変更してください。ローカルサーバーを使う場合は `http://localhost:9100/mcp` に変更します。

## 注意事項

- フック機能がないため、sourceの自動注入はありません
- Skillによる読み上げ制御もありません（ツールは手動で呼び出す形になります）
- 音声読み上げの基本機能（speak/status）は全ツールで利用可能です
