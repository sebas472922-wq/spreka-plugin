...
### Windows: Hookが自動登録されない

Windows環境では、プラグインのHook（SessionStart, PreToolUse, Stop）が自動登録されない場合があります。
MCPツール（speak, status, set_voice）は正常に動作するがHookが動作しない場合、以下の手動設定で回避できます。

`~/.claude/settings.json` に以下を追加してください（パスは環境に合わせて変更不要です）:
